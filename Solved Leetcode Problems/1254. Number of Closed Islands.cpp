...
public:
    ...
    bool BFS (int i, int j, vector<vector<bool>> &vis, vector<vector<int>>& grid)
    {
        bool isClosed = true;
        ...
        while (!q.empty())
        {
            ...
            for (int dir = 0; dir < 4; dir++)
            {
                ...
                else if (grid[newx][newy] == 0 && !vis[newx][newy])
                {
                    q.emplace(newx, newy);
                    vis[newx][newy] = true;
                }
    ...
    int closedIsland(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < m; i++)
        {
            for (int j = 0; j < n; j++)
            {
                if (grid[i][j] == 0 && !vis[i][j] && BFS(i, j, vis, grid))
                    count++;
            }
        }
    ...
    bool DFS (int x, int y, vector<vector<bool>> &vis, vector<vector<int>>& grid)
    {
        ...
        for (int dir = 0; dir < 4; dir++)
        {
            ...
            if (newx < 0 || newx >= m || newy < 0 || newy >= n)
                ...
                isClosed &= DFS(newx, newy, vis, grid);
        }
...
public:
    ...
    int closedIsland(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < m; i++)
        {
            for (int j = 0; j < n; j++)
            {
                if ((i * j == 0 || i == m - 1 || j == n - 1) && grid[i][j] == 0 && !vis[i][j])
                    DFS(i, j, vis, grid);
            }
        }
...
public:
    int closedIsland(vector<vector<int>>& grid) {
        ...
        for (int i = 1; i < m - 1; i++)
        {
            for (int j = 1; j < n - 1; j++)
            {
                if (grid[i][j] == 0 && dsu.findPar(i * n + j) != dsu.findPar(boundary))
                {
                    dsu.UnionRank(i * n + j, boundary);
                    count++;
                }
